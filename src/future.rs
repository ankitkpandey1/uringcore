...
impl UringFuture {
    ...
    fn set_result(slf: Py<Self>, py: Python<'_>, result: PyObject) -> PyResult<()> {
        let (state, callbacks, loop_) = {
             ...
             (refs.state.clone(), refs.callbacks.clone(), refs.loop_.clone_ref(py))
        };
    ...
    fn add_done_callback(slf: Py<Self>, py: Python<'_>, func: PyObject, context: Option<PyObject>) -> PyResult<()> {
         let (state, callbacks, loop_) = {
             let refs = slf.borrow(py);
             (refs.state.clone(), refs.callbacks.clone(), refs.loop_.clone_ref(py))
        };
    ...
    fn __next__(slf: Py<Self>, py: Python<'_>) -> PyResult<Option<PyObject>> {
        ...
        match &*state {
            FutureState::Pending => {
                ...
                Ok(Some(slf.to_object(py)))
            }
            FutureState::Finished(res) => {
                Err(PyStopIteration::new_err(res.clone_ref(py)))
            }
            FutureState::Failed(exc) => {
...
impl UringFuture {
    fn _schedule_callbacks(py: Python<'_>, callbacks: Arc<Mutex<Vec<(PyObject, Option<PyObject>)>>>, loop_: PyObject, future_obj: PyObject) -> PyResult<()> {
    ...
    fn _schedule_single(py: Python<'_>, loop_: PyObject, func: PyObject, future_obj: PyObject, context: Option<PyObject>) -> PyResult<()> {
