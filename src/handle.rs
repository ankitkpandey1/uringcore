...
impl UringHandle {
    ...
    fn new(callback: PyObject, args: Py<PyTuple>, loop_: PyObject, context: Option<PyObject>) -> Self {
        Self {
            callback,
            args,
            loop_,
    ...
    fn _run(&self, py: Python<'_>) -> PyResult<()> {
        ...
        if let Some(ctx) = &self.context {
            ...
            let mut run_args_vec = Vec::with_capacity(1 + args_ref.len());
            run_args_vec.push(self.callback.clone_ref(py));
            for item in args_ref.iter() {
                run_args_vec.push(item.to_object(py));
            }
            let run_args = PyTuple::new_bound(py, run_args_vec);
